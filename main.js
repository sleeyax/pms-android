...
// monkey patch conole.log() & console.error() so we automatically also log to the frontend UI when called
...
async function init() {
	...
	serverPort = await getPort({ port: userConfig.port })
	...
	router.get('/api', (req, res) => {
		...
		if (addon[method] && method != 'init') {
			addon[method](addon.getManifest(name), decodeURIComponent(query.payload)).then(resp => {
				...
				console.error(err)
				res.end(JSON.stringify({ err: 'handler error', ex: err }))
			})
		} else
			fail(res)
	})
